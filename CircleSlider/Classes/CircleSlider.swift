...
open class CircleSlider: UIControl {
  ...
  open var valueChanged: ((Float) -> Void)?
  open var value: Float {
    ...
    open func setVal(_ val: Float) {
        var vl = val
        if vl < 0 {
            vl = 0
        }
        self.value = vl
        self.latestDegree = Double(vl*180) - 90
        self.redraw()
    }
